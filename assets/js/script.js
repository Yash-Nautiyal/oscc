...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const faqButtons = document.querySelectorAll('.btn-group .btn[data-category]');

    faqButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            const category = this.getAttribute('data-category');
            const targetFaq = document.getElementById(category + '-faqs');
            ...
            allFaqCategories.forEach(function (category) {
                category.classList.add('d-none');
            });

            // Show selected FAQ category with animation
            if (targetFaq) {
                targetFaq.classList.remove('d-none');
                targetFaq.classList.add('fade-in');

                // Remove animation class after animation completes
                setTimeout(function () {
                    targetFaq.classList.remove('fade-in');
                }, 500);
            }
        ...
        // Add keyboard navigation support for FAQ buttons
    ...
    const gemFaqs = document.getElementById('gem-faqs');

    if (gemBtn && gemFaqs) {
        gemBtn.classList.add('active');
        gemFaqs.classList.remove('d-none');
    }

    // Smooth scroll to FAQ section when clicking from other parts of the page
    ...
    faqLinks.forEach(function (link) {
        link.addEventListener('click', function (e) {
            ...
            if (target === '#faq') {
                const faqSection = document.querySelector('.py-5.bg-light');
                if (faqSection) {
